...
async function produce_simple_exchange_paths(exchangeObject) {
  ...
  try {
    const cycles = await findArbitrageOpportunitiesWithTimeout(graph, 120000); // 2 minutos de timeout
    ...
    console.error("Error al buscar oportunidades de arbitraje:", error.message);
    return [];
  }
}
